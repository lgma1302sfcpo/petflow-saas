...
export function apiError(error:unknown){if(error instanceof ApiAccessError)return Response.json({error:error.message},{status:error.status});if(error instanceof Error&&"status"in error&&"code"in error&&typeof (error as {status:unknown}).status==="number")return Response.json({error:error.message,code:(error as {code:unknown}).code},{status:(error as {status:number}).status});console.error(error);return Response.json({error:"Não foi possível concluir a operação."},{status:500})}
